...
| Reviewer checks label artwork against application facts | Implemented | `src/app/page.tsx`, `src/app/api/verify/route.ts`, `src/lib/rules.ts`, `src/lib/applicationImport.ts` | CSV/JSON application imports support batch source facts; durable async queue/retry/resume remains future work. |
...
| Bottler/producer/importer name and address | Implemented | `src/lib/rules.ts`, `src/app/page.tsx`, `README.md`, `src/lib/rules.test.ts`, `public/evals/fixtures/html-generated/` | V1 now checks standard attribution/importer phrases; durable audit trail remains future work. |
